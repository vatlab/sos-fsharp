...
#        assert abs(float(val) - float(self.put_to_SoS(notebook, val))) < 1e-10
    ...
    def test_get_num_array(self, notebook):
        assert '[1] |> ResizeArray' == self.get_from_SoS(notebook, '[1]')
        assert '[1 ; 2] |> ResizeArray' == self.get_from_SoS(notebook, '[1, 2]')
        #
        assert '[1.23] |> ResizeArray' == self.get_from_SoS(notebook, '[1.23]')
        assert '[1.4; 2] |> ResizeArray' == self.get_from_SoS(notebook, '[1.4, 2]')
    ...
    def test_put_num_array(self, notebook):
        ...
        assert '1' == self.put_to_SoS(notebook, '[1] |> ResizeArray')
        assert '[1, 2]' == self.put_to_SoS(notebook, '[1 ;2] |> ResizeArray')
        #
        assert '1.23' == self.put_to_SoS(notebook, '[1.23] |> ResizeArray')
        assert '[1.4, 2]' == self.put_to_SoS(notebook, '[1.4; 2] |> ResizeArray')
    ...
    def test_get_logic_array(self, notebook):
        assert '[true] |> ResizeArray' == self.get_from_SoS(notebook, '[True]')
        assert '[true; false; true] |> ResizeArray' == self.get_from_SoS(notebook,
                                                      '[True, False, True]')
        # assert 'TRUE' == self.get_from_SoS(notebook, '[True]')
    ...
    def test_put_logic_array(self, notebook):
        # Note that single element numeric array is treated as single value
        assert 'True' == self.put_to_SoS(notebook, '[true] |> ResizeArray')
        assert '[True, False, True]' == self.put_to_SoS(notebook,
                                                        '[true; false; true] |> ResizeArray')
    ...
    def test_put_str(self, notebook):
        ...
        assert r"'ab\td'" == self.put_to_SoS(notebook, '"ab\\td"')
        # assert "'ab c d'" == self.put_to_SoS(notebook, "'ab c d'")
        # assert "'ab\\td'" == self.put_to_SoS(notebook, r"'ab\td'")

    def test_get_mixed_list(self, notebook):
        assert '[1.4 :> obj; TRUE :> obj; "asd":> obj] |> ResizeArray' == self.get_from_SoS(notebook,
                                                       '[1.4, True, "asd"]')
    ...
    def test_get_dict(self, notebook):
        ...
        assert '[("a",1); ("b",2); ("c",3)] |> Map.ofList |> System.Collections.Generic.Dictionary' == self.get_from_SoS(
    ...
    def test_put_named_list(self, notebook):
        assert "{'a': 1, 'b': 2, 'c': '3'}" == self.put_to_SoS(
            notebook, '[("a",1); ("b",2); ("c",3)] |> Map.ofList |> System.Collections.Generic.Dictionary')
        # assert "{'a': 1, 'b': 2, 'c': '3'}" == self.put_to_SoS(
        #     notebook, "list(a=1, b=2, c='3')")

    def test_get_set(self, notebook):
        output = self.get_from_SoS(notebook, "{1.5, 'abc'}")
        assert '[ 1.5 :> obj; "abc" :> obj ] |> System.Collections.Generic.HashSet' == output or '[  "abc" :> obj; 1.5 :> obj ] |> System.Collections.Generic.HashSet' == output
        # output = self.get_from_SoS(notebook, "{1.5, 'abc'}")
        # assert "1.5\n'abc'" == output or "'abc'\n1.5" == output

    def test_put_unnamed_list(self, notebook):
        output = self.put_to_SoS(notebook, '[ 1.5 :> obj; "abc" :> obj ]')
        assert "[1.5, 'abc']" == output or "['abc', 1.5]" == output
        # output = self.put_to_SoS(notebook, "list(1.5, 'abc')")
        # assert "[1.5, 'abc']" == output or "['abc', 1.5]" == output

#https://docs.microsoft.com/en-us/dotnet/api/system.numerics.complex?view=netframework-4.8
    def test_get_complex(self, notebook):
        assert "new System.Numerics.Complex(1,2.2)" == self.get_from_SoS(notebook, "complex(1, 2.2)")
    ...
    def test_put_complex(self, notebook):
        assert "(1+2.2j)" == self.put_to_SoS(notebook,
                                             "new System.Numerics.Complex(1,2.2)")
        # assert "(1+2.2j)" == self.put_to_SoS(notebook,
        #                                      "complex(real=1, imaginary=2.2)")

    def test_get_recursive(self, notebook):
        assert 'dict [ ("a", 1 :> obj); ("b", (dict [ ("c", 3 :> obj ); ("d", "whatver" :> obj )]) :> obj ]' == self.get_from_SoS(
    ...
    def test_put_recursive(self, notebook):
        assert "{'a': 1, 'b': {'c': 3, 'd': 'whatever'}}" == self.put_to_SoS(
            notebook, 'dict [ ("a", 1 :> obj); ("b", (dict [ ("c", 3 :> obj ); ("d", "whatver" :> obj )]) :> obj ]')
        # assert "{'a': 1, 'b': {'c': 3, 'd': 'whatever'}}" == self.put_to_SoS(
    ...
    def test_get_matrix(self, notebook):
        ...
        assert "[| [| 1; 2 |]; [|3; 4 |] |]" == self.get_from_SoS(notebook,
                                                    'np.matrix([[1,2],[3,4]])')
        # notebook.call('import numpy as np', kernel='SoS')
    ...
    #TODO: equivalent issues/constructs do not exist?
